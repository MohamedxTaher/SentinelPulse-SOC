...
SSH_FAILURE = (
    "Mar 14 08:12:01 webserver sshd[1234]: "
    "Failed password for root from 192.168.1.100 port 22 ssh2"
)
SSH_SUCCESS = (
    "Mar 14 08:15:00 webserver sshd[1235]: "
    "Accepted password for alice from 10.0.0.5 port 45678 ssh2"
)


def test_detects_auth_log():
    assert detect_log_type(SSH_FAILURE) in ("auth", "syslog")


def test_detects_apache_log():
    line = (
        '192.168.1.1 - - [14/Mar/2024:10:00:01 +0000] '
...
def test_unknown_line_returned_safely():
    entry = parse_line("this is not a real log line")
    assert entry["type"] == "unknown"
    assert entry["suspicious"] is False


def test_apache_user_agent_captured():
    line = (
        '45.33.32.156 - - [02/Sep/2026:12:00:44 +0000] '
        '"GET /search?q=1 HTTP/1.1" 200 2690 "-" "sqlmap/1.7"'
    )
    entry = parse_line(line)
    assert entry["type"] == "apache"
    assert entry["user_agent"] == "sqlmap/1.7"
